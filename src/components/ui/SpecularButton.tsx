...
export const SpecularButton: React.FC<SpecularButtonProps> = ({
  ...
  // Lazy WebGL init — only create the WebGL context when the button is visible
  useEffect(() => {
    ...
    const update = (now: number) => {
      if (!isVisible || !initialized) { raf = 0; return; }
      ...
      program.uniforms.uRadius.value = Math.min(p.radius, Math.min(sizeRef.w, sizeRef.h) / 2) * (window.devicePixelRatio || 1);
      program.uniforms.uLineColor.value = [lineC.r, lineC.g, lineC.b];
      program.uniforms.uBaseColor.value = [baseC.r, baseC.g, baseC.b];
      program.uniforms.uIntensity.value = p.intensity * bright;
      program.uniforms.uShineSize.value = (p.shineSize * Math.PI) / 180;
      program.uniforms.uShineFade.value = (p.shineFade * Math.PI) / 180;
      program.uniforms.uThickness.value = p.thickness * (window.devicePixelRatio || 1);
    ...
    const initWebGL = () => {
      ...
      try {
        ...
        console.warn('SpecularButton WebGL initialization fallback:', e);
      }
    };

    // Use IntersectionObserver to lazily init WebGL only when visible
    const io = new IntersectionObserver((entries) => {
      for (const entry of entries) {
        isVisible = entry.isIntersecting;
        if (isVisible) {
          if (!initialized) initWebGL();
          else if (!raf) { last = performance.now(); raf = requestAnimationFrame(update); }
        } else {
